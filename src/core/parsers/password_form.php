<form id="passResetForm" class="form col-12" action="index.html" method="post">
  ...
  &nbsp;
  <br>
  <input id="submitPassResetForm" type="button" class="btn btn-sm btn-default-teel btn-success col-12" onclick="postonly(path_action, {'reset_username': $('#username').val(), 'pass_reset': $('#pass_reset').val(), 'pass_confirm': $('#pass_confirm').val(), 'reset_token': $('#reset_token').val()}, 'message_err');" style="border-radius: 35px;" value="Submit"/>
