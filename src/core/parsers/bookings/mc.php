...
<div class="row form-row align-items-center">
    <div class="col-12 col-lg-6/ mb-3">
        <label for="event_description" class="form-label px-2 font-weight-bolder">Event Description</label>
    ...
    <div class="col-12 col-lg-6/ mb-3">
        <label for="event_address" class="form-label px-2 font-weight-bolder"> Event location address</label>
        <div class="input-group mb-2">
            <span class="input-group-text text-dark bg-light" style="border-right: none;"><i class="fas fa-map-marked-alt input_color"></i></span>
            <textarea class="form-control shadow-none bg-light" id="event_address" name="event_address" placeholder="Address ..." rows="2" required> <?= ((isset($event['event_address'])) ? $event['event_address'] : '') ?></textarea>
        </div>
    </div>
...
<div class="form-group">
    ...
    <label for="date_of_birth" class="form-label px-2 font-weight-bolder"> Event Date & Time</label>
...
<div class="row">
    ...
    <div class="col-12 mb-3">
        <label for="event_period" class="form-label px-2 font-weight-bolder"> Event time period </label>
        <select id="event_period" class="form-control shadow-none bg-light" name="event_period">
            <option value=""> Select event time period </option>
            <option value="half_day" <?= ((isset($event['event_period']) && $event['event_period'] == 'half_day') ? 'selected' : '') ?>> Half day (1 - 4 Hours)</option>
            <option value="full_day" <?= ((isset($event['event_period']) && $event['event_period'] == 'full_day') ? 'selected' : '') ?>> Full day (4 - 8 Hours)</option>
        </select>
    </div>
