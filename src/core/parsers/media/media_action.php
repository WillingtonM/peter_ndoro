...
$intval       = ($key == 'appearance' || $key == 'podcast') ? 4 : 3;
$feed_count   = 1;
...
// query gallery media
if ($key != 'appearance' && $key != 'podcast') {
    $rgst_sql = "SELECT * FROM media WHERE media_status = 1 AND media_type != 'appearance' AND media_type != 'podcast' ORDER BY media_publish_date DESC LIMIT $sql_pg_strt, $intval";
    $rgst_dta = [];
} else {
